...
public class DanhSachTieuChiRepository {
    private static DanhSachTieuChiRepository instance;
    public static DanhSachTieuChiRepository getInstance(){
        if (instance==null){
            instance=new DanhSachTieuChiRepository();
        }
        return instance;
    }
    private MutableLiveData<DanhSachTieuChiRespon> mutableLiveData ;
    public MutableLiveData<DanhSachTieuChiRespon> getMutableLiveData(Map<String, String> params){
        mutableLiveData = new MutableLiveData<>();
        try {

            Service service = NetContext.instance.create(Service.class);

            service.getDanhSachTieuChiGhiChu(params).enqueue(new Callback<DanhSachTieuChiRespon>() {
                @Override
                public void onResponse(Call<DanhSachTieuChiRespon> call, Response<DanhSachTieuChiRespon> response) {
                    Log.d("BBB", "onResponse: "+response.body());
                    try {
                        DanhSachTieuChiRespon danhSachThemPhanHoiRespon= response.body();
                ...
                @Override
                public void onFailure(Call<DanhSachTieuChiRespon> call, Throwable t) {
